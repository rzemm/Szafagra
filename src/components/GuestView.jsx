...
export function GuestView({ isOwner, playerDivRef, isPlaying, currentSong, remaining, queue, voteThreshold, nextOptionKeys, nextOptions, nextVotesData, userId, vote, skipThreshold, mySkipVote, voteSkip }) {
  return (
    <>
      ...
      {isPlaying && queue.length <= voteThreshold && nextOptionKeys.length === 0 && <div className="voting-card"><p className="empty-hint">Za mało piosenek na głosowanie (dodaj więcej do playlisty).</p></div>}
